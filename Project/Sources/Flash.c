...
{
        // return Flash_Erase(); // We erase the memory on setup
        // Do nothing in flash init.
        return true;
}
...
bool Flash_AllocateVar(volatile void** variable, const uint8_t size)
{
        ...
        uint8_t available = 0; //Number of available positions
        uint8_t found = 0; //Empty positions found

        while(position<8) //Loop through sector locations to find free space
        {
                if(CHECKER(Occupied,position++) == 0) //Free position
                {
                        available++; //Increase available positions if current location empty
                        found++;
                }
                else
                {
                        found = 0; //No empty positions have been found
                        continue; //Return to loop begining
                }

                if((available % size == 0) && (found == size)) //Enough available in a row spaces for data
                {
                        if(AllocateVar(variable, position-1, size)) //Allocate variable into the found free spaces
                        {
                                break;
                        }
                        else{
...
bool Flash_Write32(volatile uint32_t* const address, const uint32_t data)
{
  ...
  uint32_t start_address = (uint32_t)address; //Stores the starting address to store data

  if ((start_address/4) % 2 == 0) // Setting up a phrase
  {

    phrase.s.Lo = data; //Combine data in address+4 with current word
    phrase.s.Hi = _FW(start_address+4);

    return WritePhrase(start_address,phrase.l); // Return 64 bit phrase
  }
  else
  {
    phrase.s.Lo = _FW(start_address-4); //Combine data in address-4 with current word
    phrase.s.Hi = data;
    return WritePhrase(start_address-4,phrase.l); // Return 64 bit phrase
  }
...
bool Flash_Write16(volatile uint16_t* const address, const uint16_t data)
{
  ...
  if(start_address % 4 ==0)
  {
    ...
    word.s.Hi = _FH(start_address+2);
    return Flash_Write32(&(_FW(start_address)),word.l); // Return 32 bit word
  }
  else
  {
    word.s.Lo = _FH(start_address-2); //Combine data in address-2 with current half word
    word.s.Hi = data;
    return Flash_Write32(&(_FW(start_address-2)),word.l); // Return 32 bit word
  }
...
bool Flash_Write8(volatile uint8_t* const address, const uint8_t data)
{
  ...
  if(start_address % 2 ==0)
  {
    ...
    halfword.s.Hi = _FB(start_address+1);
    return Flash_Write16(&(_FH(start_address)),halfword.l); // Return 16 bit halfword
  }
  else
  {
    halfword.s.Lo = _FB(start_address-1); //Combine data in address-1 with current byte
    halfword.s.Hi = data;
    return Flash_Write16(&(_FH(start_address-1)),halfword.l); // Return 16 bit halfword
  }
